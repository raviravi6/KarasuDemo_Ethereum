...
contract ERC20EscrowCounterSimple is Ownable {
    ...
    event PayeeSet(uint256 indexed depositId, address payee, string action);
    ...
    function deposit(
        ...
        expirations[newDepositId][address(token)] =
            block.timestamp +
            _expiration;
        emit Deposited(newDepositId, address(token), _amount, "deposited");
    }

    function setPayee(uint256 _depositId, address payable _payee)
        public
        onlyOwner
    {
        payees[_depositId] = _payee;
        emit PayeeSet(_depositId, _payee, "payeeSet");
    }
    ...
    function withdraw(
        ...
        emit Withdrawn(_depositId, address(token), _amount, "completed");
    }

    function refund(uint256 _depositId, IERC20 token) public onlyOwner {
        require(
        ...
        emit Withdrawn(_depositId, address(token), payment, "refunded");
    }
}
