...
from brownie import MainEscrow, config, network
from web3 import Web3
import time


def deploy_mainEscrow(account, token):
    main_escrow = MainEscrow.deploy(
        token,
        {"from": account},
...
def main():
    ...
    # main_escrow = deploy_mainEscrow(account, dai_token)
    main_escrow = MainEscrow[-1]
    amount = Web3.toWei(10, "ether")
    expiration_sec = 10

    approve_token_transfer(token, main_escrow.address, account, amount)
    deposit_to_escrow(main_escrow, account, non_owner, amount, expiration_sec)
    withdraw_from_escrow(main_escrow, account, non_owner, amount)
